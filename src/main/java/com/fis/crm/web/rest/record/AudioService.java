...
public class AudioService {
    ...
    public ResponseEntity<byte[]> prepareContent(String fileName, String fileType, String range) {
        ...
        try {
            ...
            if (range == null) {
                return ResponseEntity.status(HttpStatus.OK)
                        .header(Constants.AUDIO.CONTENT_TYPE, Constants.AUDIO.AUDIO_CONTENT + fileType)
                        .header(Constants.AUDIO.CONTENT_LENGTH, String.valueOf(fileSize))
        ...
        return ResponseEntity.status(HttpStatus.PARTIAL_CONTENT)
                .header(Constants.AUDIO.CONTENT_TYPE, Constants.AUDIO.AUDIO_CONTENT + fileType)
                .header(Constants.AUDIO.ACCEPT_RANGES, Constants.AUDIO.BYTES)
                .header(Constants.AUDIO.CONTENT_LENGTH, contentLength)
                .header(Constants.AUDIO.CONTENT_RANGE, Constants.AUDIO.BYTES + " " + rangeStart + "-" + rangeEnd + "/" + fileSize)
                .body(data);
    ...
    public byte[] readByteRange(String filename, long start, long end) throws IOException {
        ...
        try (InputStream inputStream = (Files.newInputStream(path));
            ...
            byte[] data = new byte[Constants.AUDIO.BYTE_RANGE];
            int nRead;
    ...
    private String getFilePath() {
        URL url = this.getClass().getResource(Constants.AUDIO.RECORD);
